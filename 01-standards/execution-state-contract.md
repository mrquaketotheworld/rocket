...
`TODO.md` — необязательный transient-файл в корне репозитория для текущей работы агента.

Контракт:

- хранит только пункты, назначенные сейчас;
- не является canonical truth, backlog или историей проекта;
- не должен содержать выполненный глобальный список после закрытия задачи;
- если все пункты отмечены выполненными, содержимое файла очищается;
- если нет задач `in_progress`, файл должен быть пустым;
- источник истины по задачам остаётся `task-registry.edn` + файлы `TASK-*`.
...
- `current-state.edn :active_tasks` совпадает с задачами `in_progress`;
- `TODO.md` пуст, если нет активных задач, и очищен, если все пункты выполнены.
